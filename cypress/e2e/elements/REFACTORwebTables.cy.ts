...
import WebTablesPage from '../../pageObjects/webTables/REFACTORWebTablesPage';
...
describe('Visualizar Dados', () => {
	...
	it('Verifica se os dados estão sendo apresentados na tebela', () => {
		cy.fixture('/webTables/data').then((datas) => {
			for (let i in datas) {
				WebTables.rowTable(parseInt(i)).within(($el) => {
					expect($el).to.contain(datas[i].firstname);
					expect($el).to.contain(datas[i].lastname);
					expect($el).to.contain(datas[i].age);
					expect($el).to.contain(datas[i].salary);
					expect($el).to.contain(datas[i].department);
				});
			}
		});
	});
...
describe('Pesquisar Dados', () => {
	...
	describe('Pesquisar por Firstname', () => {
		it.only('Pesquisa com string completa', () => {
...
describe('Adicionar Usuário', () => {
	describe('Partição Válida', () => {
		...
		it('Preencher todos os campos com dados válidos', () => {
			...
			//TODO: Verificar se os dados foram apresentados com Within
		});
	});
